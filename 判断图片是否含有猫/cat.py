...
def initialize_parameters_deep(layer_dims):
    ...
    #print("随机初始化参数为：",parameters)
    #print("随机初始化参数长度为：",len(parameters))
    return parameters
#正向传播
...
def linear_activation_forward(A_prev, W, b, activation):
    ...
    # linear_cache指的是l层对应的输入A[l -1], 以及W[l], b[l]，也就是当前层的参数；
...
def L_model_forward(X, parameters):
    ...
    #输入
    A = X
    # 获取神经网络层数, // 表示整除，因为有W和b两个参数，所以要除以2.
    L = len(parameters)// 2
    # 实现 [LINEAR -> RELU]*(L-1).
...
def compute_cost(AL, Y):
    ...
    # 还要确保计算出来的 cost 是一个数, 而不是矩阵。使用cost = np.squeeze(cost)
    return cost
#反向传播
